...
=> 연결된 동안 수행한 작업 정보를 계속 서버에서 유지한다. 
   그래서 상태 정보를 유지한다는 의미로 "stateful" 방식이라고도 한다.
=> 장점
   - 클라이언트의 작업 정보를 유지하기 때문에 영속성 있는 작업이 처리가 가능하다.
=> 단점
   - 클라이언트가 연결되면 그 클라이언트 접속 정보를 계속 서버 메모리에 유지해야 하기 때문에
     메모리를 많이 차지한다.
   - 많은 클라이언트의 요청을 처리할 수 없다.
   - 클라이언트가 연결을 끊지 않으면 서버는 그 클라이언트의 정보를 계속 유지해야 한다.
   - 메모리 낭비가 심하다.
=> 예)
   FTP, 채팅, 게임, 화상통신, Telnet 등  
   자바 예제로 만든 EchoClient/EchoServer, GameClient/GameServer 
   
2) connectionless(=stateless)
=> 서버에 작업을 요청할 때 마다 연결하고 응답을 받으면 즉시 연결을 끊는다.
=> 연결하기 --> 요청 --> 응답 --> 연결끊기, 연결하기 --> 요청 --> 응답 --> 연결끊기 
